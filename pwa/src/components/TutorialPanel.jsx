...
import { 
  ...
  ChevronDown, ChevronUp, Check, MousePointerClick, Smartphone, Apple
} from 'lucide-react';

export default function TutorialPanel({ userType = 'coach' }) {
    ...
    <div className="flex flex-col gap-6 animate-fade-in w-full max-w-4xl mx-auto">
      <div className="glass-card rounded-2xl p-6 shadow-lg border border-zinc-800">
        ...
        <p className="text-sm text-zinc-400">
          {userType === 'coach' 
            ? 'Bienvenido a la guía rápida. Aquí aprenderás a instalar la app, gestionar a tus alumnos, crear rutinas y llevar el control de tus finanzas.'
            : 'Bienvenido a la guía rápida. Aquí aprenderás cómo instalar la aplicación en tu celular para tener acceso directo a tus entrenamientos.'}
        </p>
      </div>

      {/* SECCIÓN 0: INSTALAR APP */}
      <div className="glass-card rounded-2xl shadow-lg border border-zinc-800 overflow-hidden">
        <div 
          ...
          onClick={() => toggleSection('instalar')}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-zinc-100">Instalar Syncro como App</h3>
              <p className="text-xs text-zinc-500">Agrega un acceso directo a la pantalla de inicio de tu celular</p>
            </div>
          </div>
          {openSections.instalar ? <ChevronUp className="text-zinc-400" /> : <ChevronDown className="text-zinc-400" />}
        </div>
        
        {openSections.instalar && (
          <div className="p-6 border-t border-zinc-800 bg-zinc-900/20 flex flex-col gap-6">
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Android */}
              <div className="p-5 bg-zinc-900/60 border border-zinc-800 rounded-xl flex flex-col gap-3">
                <div className="flex items-center gap-2 mb-2">
                  <Smartphone className="w-5 h-5 text-emerald-400" />
                  <h4 className="font-bold text-zinc-200 text-base">En Android (Google Chrome)</h4>
                </div>
                <ol className="list-decimal pl-5 space-y-3 text-sm text-zinc-400">
                  <li>Abre Syncro desde el navegador <strong>Google Chrome</strong>.</li>
                  <li>Toca el icono de los <strong>tres puntitos verticales</strong> en la esquina superior derecha del navegador.</li>
                  <li>Busca y selecciona la opción <strong>"Agregar a la pantalla principal"</strong> o <strong>"Instalar aplicación"</strong>.</li>
                  <li>Confirma tocando en <strong>Agregar</strong>. ¡Listo! Ahora verás el ícono de Syncro junto a tus otras apps.</li>
                </ol>
              </div>
              ...
              <div className="p-5 bg-zinc-900/60 border border-zinc-800 rounded-xl flex flex-col gap-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h4 className="font-bold text-zinc-200 text-base">En iPhone (Safari)</h4>
                </div>
                <ol className="list-decimal pl-5 space-y-3 text-sm text-zinc-400">
                  <li>Abre Syncro desde el navegador <strong>Safari</strong> (es obligatorio usar Safari).</li>
                  <li>Toca el icono de <strong>Compartir</strong> en la barra inferior (el cuadrado con una flecha apuntando hacia arriba).</li>
                  <li>Desliza el menú hacia abajo y selecciona <strong>"Agregar a inicio"</strong> (Add to Home Screen).</li>
                  <li>Toca <strong>Agregar</strong> en la esquina superior derecha. ¡Listo! Ya tienes la app en tu pantalla de inicio.</li>
                </ol>
              </div>
            </div>
